          ...
          {comps.map((c, i) => (
            <div key={i}>
              <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 7 }}>
                <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMID }}>{c.label}</span>
  ...
  return (
    <div style={{ background: eCARD, border: "1px solid " + eLINE, borderRadius: 16, overflow: "hidden", opacity: locked ? 0.85 : 1, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: 18, display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMUT, marginBottom: 6 }}>Based on {r.based.length === 1 ? "1 assessment" : `${r.based.length} assessments`}:</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 14 }}>
          {r.based.map((b, i) => {
            const isDone = i < r.doneCount;
            return <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, padding: "4px 10px", borderRadius: 2, background: isDone ? "color-mix(in srgb, #14853D 15%, var(--card))" : "var(--status-neutral-bg)", border: "1px solid " + (isDone ? "#14853D" : "#94918C"), color: "var(--ink)" }}>{isDone && <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M5 13l4 4L19 7" /></svg>}{b}</span>;
          })}
        </div>
...
function ReportCover({ report, pages }) {
  return (
    <div>
      ...
      <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, letterSpacing: 0.2, color: eBLUE, marginBottom: 10 }}>Marsh · Confidential</div>
      ...
      <div style={{ borderTop: "1px solid " + eLINE, paddingTop: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMUT }}>Prepared for</span><span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMID }}>{LH.user.first} {LH.user.last}</span></div>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMUT }}>Based on</span><span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMID }}>{report.based.join(", ")}</span></div>
        <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMUT }}>Length</span><span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMID }}>{pages} pages</span></div>
      </div>
    </div>
  );
}
...
function ReportPage({ report, page }) {
  ...
  const Eyebrow = ({ children }) => <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, letterSpacing: 0.2, color: eBLUE, marginBottom: 10 }}>{children}</div>;
  const t = (page - 1) % 5;
        ...
        {comps.map((c, i) => (
          <div key={i}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMID }}>{c.name}</span>
  ...
  return (
    <div style={{ maxWidth: "var(--content-max)", margin: "36px var(--fol-mx) 72px", padding: 0 }}>
      ...
      <p className="ed-insights-banner" style={{ fontFamily: "var(--sans)", fontSize: 15, color: eINK, lineHeight: 1.7, margin: 0, maxWidth: "none", whiteSpace: "nowrap" }}>
        <span style={{ display: "inline-block", fontSize: 15, fontWeight: 400, color: "var(--ink)", background: "color-mix(in srgb, var(--accent) 15%, var(--card))", border: "1px solid var(--accent)", padding: "4px 10px", borderRadius: 2, marginRight: 9, verticalAlign: "1px", whiteSpace: "nowrap" }}>{availCount} of {reps.length} ready</span>
...
function EdScheduling({ onBack, initialCenter, demo, onView }) {
    ...
    return (
      <div style={{ maxWidth: "var(--content-max)", margin: "36px var(--fol-mx) 72px", padding: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 15, marginBottom: 18 }}>
          <div style={{ width: 50, height: 50, borderRadius: "50%", background: "color-mix(in srgb, " + c.color + " 12%, var(--card))", border: "1px solid color-mix(in srgb, " + c.color + " 24%, transparent)", color: c.color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Ic size={23} /></div>
          <div>
            ...
            return (
              <div key={slot.id} className="ed-slot-row" style={{ background: isBooked ? "rgba(20,133,61,.05)" : eCARD, border: "1px solid " + (isBooked ? "rgba(20,133,61,.3)" : eLINE), borderRadius: 14, padding: "16px 18px", opacity: full ? 0.55 : 1, display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ textAlign: "center", width: 54, flexShrink: 0 }}>
                  <div style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMUT, letterSpacing: 0.2 }}>{slot.day.slice(0, 3)}</div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    ...
                    {slot.cancelBefore ? (
                      <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: "var(--ink)", background: "color-mix(in srgb, var(--accent) 15%, var(--card))", border: "1px solid var(--accent)", padding: "4px 10px", borderRadius: 2 }}>Cancel OK ({slot.cancelBefore})</span>
                    ) : (
                      <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: "var(--ink)", background: "color-mix(in srgb, #C53532 15%, var(--card))", border: "1px solid #C53532", padding: "4px 10px", borderRadius: 2 }}>No cancellation</span>
                    )}
                  </div>
                </div>
                <div className="ed-slot-action" style={{ flexShrink: 0 }}>
                  {isBooked ? (
                    ...
                    <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMUT }}>Full</span>
                  ) : (
                  ...
                  {dayEvents.map((e, ei) => {
                    ...
                    return (
                      <div key={ei} onClick={(ev) => { const blockRect = ev.currentTarget.getBoundingClientRect(); const wrap = calWrapRef.current; const wRect = wrap ? wrap.getBoundingClientRect() : { left: 0, top: 0, width: 1 }; const scale = (wrap && wrap.offsetWidth) ? (wRect.width / wrap.offsetWidth) : 1; setPop({ slot: e.slot, center: e.center, program: e.program, booked: isBooked, relLeft: (blockRect.left - wRect.left) / scale, relRight: (blockRect.right - wRect.left) / scale, relTop: (blockRect.top - wRect.top) / scale }); }}
                        style={{ position: "absolute", top: top + 2, left: 4, right: 4, height, background: "color-mix(in srgb, " + tint + " 9%, var(--card))", borderLeft: "3px solid " + tint, borderRadius: 8, padding: "7px 9px", cursor: "pointer", overflow: "hidden", transition: "background .15s" }}
                        onMouseEnter={(ev) => ev.currentTarget.style.background = "color-mix(in srgb, " + tint + " 16%, var(--card))"}
                        onMouseLeave={(ev) => ev.currentTarget.style.background = "color-mix(in srgb, " + tint + " 9%, var(--card))"}>
                ...
                {pop.booked ? (
                  ...
                  <React.Fragment>
                    ...
                    <button onClick={() => { setBooked((b) => ({ ...b, [slot.id]: true })); markCenterReserved(center); setDoneSlot({ slot, center, program: pop.program, langPref, lang, justBooked: true }); setPop(null); }}
                      style={{ width: "100%", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, background: "var(--action)", color: "var(--action-text)", border: "none", borderRadius: 9, padding: "10px", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>Book this slot <I.check size={14} /></button>
                  </React.Fragment>
                )}
              </div>
            </React.Fragment>
  ...
  const modals = (
    <React.Fragment>
      ...
      {schedToast && (
        <div style={{ position: "fixed", left: "50%", bottom: 28, transform: "translateX(-50%)", zIndex: 90, background: eGOLD, color: "var(--action-text)", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, padding: "11px 18px", borderRadius: 10, boxShadow: "0 10px 30px rgba(0,15,71,.28)", display: "inline-flex", alignItems: "center", gap: 8 }}>
  ...
  const renderBooked = () => {
    ...
    const row = (label, value) => (
      <div style={{ display: "flex", gap: 20, padding: "14px 0", borderTop: "1px solid " + eLINE }}>
        ...
        <div style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMID, fontWeight: 400 }}>{value}</div>
      </div>
    );
    return (
      <div style={{ maxWidth: "var(--content-max)", margin: "36px var(--fol-mx) 72px", padding: 0 }}>
                  ...
                  <div key={i} onClick={() => setDoneSlot({ ...b, justBooked: false })} title="View booking details"
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }} onClick={(e) => e.stopPropagation()}>
                      ...
                      {b.slot.cancelBefore ? (
                        ...
                        <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMUT }}>Non-cancellable</span>
                      )}
                  ...
                  return (
                    <div key={c.id} onClick={() => setView(c.id)} style={{ display: "flex", flexDirection: "column", height: "100%", minWidth: 0, boxSizing: "border-box", background: eCARD, border: "1px solid " + eLINE, borderRadius: 16, padding: "16px 18px", cursor: "pointer", boxShadow: "0 1px 3px rgba(0,15,71,.04)", transition: "box-shadow .15s, transform .15s" }}
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 13, marginBottom: 12 }}>
                        ...
                        {bookedInCenter > 0 && <span style={{ fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: "var(--ink)", background: "color-mix(in srgb, #14853D 15%, var(--card))", border: "1px solid #14853D", padding: "4px 10px", borderRadius: 2, display: "inline-flex", alignItems: "center", gap: 4, flexShrink: 0 }}><I.check size={11} /> {bookedInCenter} booked</span>}
                      </div>
