/* ════════════════════════════════════════════════════════════════════
   lh-loader.js  —  in-page sign-in loader (Generali build)
   Shows a branded "signing you in" overlay ON TOP of the login / sign-up
   screen (the form stays visible, dimmed + blurred behind it), then the
   caller redirects to the dashboard.

   Brand-aware: all colours come from the live CSS tokens (--accent /
   --primary), so the overlay follows Generali's palette (red mark + ring
   on a neutral-dark scrim) and stays correct in light or dark mode.
...
(function () {
  ...
  function injectStyles() {
    ...
    css.textContent =
      ...
      "background:rgba(0,0,0,.55);-webkit-backdrop-filter:blur(7px) saturate(1.05);backdrop-filter:blur(7px) saturate(1.05);" +
      ...
      "#lh-loader .lh-l-ring circle{fill:none;stroke:var(--accent,#AA1B17);stroke-width:2.5;stroke-linecap:round;}" +
      "#lh-loader .lh-l-flash{position:absolute;width:96px;height:96px;border-radius:50%;background:radial-gradient(circle,color-mix(in srgb, var(--accent,#AA1B17) 55%, transparent),transparent 68%);opacity:0;transform:scale(.4);animation:lhl-flash .55s ease-out .78s;}" +
      ...
      "#lh-loader .lh-l-mark .lh-l-right{fill:var(--primary,#81A0BD);animation:lhl-in-r .8s cubic-bezier(.16,1,.3,1) .12s forwards;}" +
      "#lh-loader .lh-l-mark .lh-l-left{fill:var(--primary,#81A0BD);animation:lhl-in-l .8s cubic-bezier(.16,1,.3,1) .12s forwards;}" +
      ...
      "#lh-loader .lh-l-bar i{display:block;height:100%;width:40%;border-radius:2px;background:var(--accent,#AA1B17);animation:lhl-slide 1.15s ease-in-out infinite;}" +
    ...
    el.innerHTML =
      '<div class="lh-l-stage">' +
        ...
        '<svg class="lh-l-mark" viewBox="0 0 43.17 44.26" aria-hidden="true">' +
          '<polygon class="lh-l-right" points="42.49 0 21.65 30.43 22.2 30.43 35.07 24.39 35.07 44.26 43.17 44.26 43.17 0 42.49 0"></polygon>' +
          '<polygon class="lh-l-left" points="0 0 0 44.26 8.1 44.26 8.1 24.4 20.9 30.43 21.52 30.43 .68 0 0 0"></polygon>' +
        '</svg>' +
      '</div>' +
