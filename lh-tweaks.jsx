...
function LighthouseTweaks() {
  ...
  const DEFAULTS = { accent: "var(--accent)", action: "#000F47", canvas: "#F7F3EE", heading: "Sans", primary: "#000F47", sidebar: "Midnight", align: (typeof document !== "undefined" && getComputedStyle(document.documentElement).getPropertyValue("--fol-mx").trim() === "auto") ? "Center" : "Left", theme: (typeof localStorage !== "undefined" && localStorage.getItem("lh-theme") === "dark") ? "Dark" : "Light", device: (typeof localStorage !== "undefined" && localStorage.getItem("lh-device")) || "Desktop" };
  ...
  React.useEffect(() => {
    ...
    // DGE (light mode): a WHITE side menu with dark ink and a subtle steel active
    // highlight, rather than a steel-filled rail. (Dark mode keeps the dark rail above.)
    if (((window.LHBrand && window.LHBrand.current()) || "marsh") === "dge") {
      set({ "--rail-bg": "#FFFFFF", "--rail-fg": "rgba(0,0,0,.80)", "--rail-icon": "rgba(0,0,0,.68)", "--rail-active-bg": "color-mix(in srgb, var(--primary) 24%, transparent)", "--rail-active-fg": "#1A1A1A", "--rail-active-icon": "#1A1A1A", "--rail-group": "#6B6A64", "--rail-border": "rgba(0,0,0,.12)", "--rail-ring-track": "rgba(0,0,0,.16)", "--rail-cta-bg": "#1A1A1A", "--rail-cta-fg": "#ffffff", "--rail-cta-border": "transparent", "--rail-active-line": "rgba(0,0,0,.12)", "--rail-logo-white": "none", "--rail-logo-dark": "block" });
      return;
    }
    if (t.sidebar === "White") {
    ...
    if (relLum(toRGB(primary)) > 0.18) {
      ...
      set({ "--rail-bg": "var(--primary)", "--rail-fg": "rgba(255,255,255,.8)", "--rail-icon": "rgba(255,255,255,.75)", "--rail-active-bg": "rgba(255,255,255,.16)", "--rail-active-fg": "#ffffff", "--rail-active-icon": "#ffffff", "--rail-group": "rgba(255,255,255,.9)", "--rail-border": "rgba(255,255,255,.12)", "--rail-ring-track": "rgba(255,255,255,.28)", "--rail-cta-bg": "rgba(255,255,255,.95)", "--rail-cta-fg": "#0B1220", "--rail-cta-border": "transparent", "--rail-active-line": "rgba(255,255,255,.10)", "--rail-logo-white": "block", "--rail-logo-dark": "none" });
    }
    ...
    <React.Fragment>
      ...
      <div className="lh-net-overlay" role="alert" aria-live="assertive"
        ...
        <div style={{ textAlign: "center", maxWidth: 420, padding: "0 8px" }}>
          ...
          <button onClick={() => setOffline(false)} style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "var(--primary, #000F47)", color: "var(--action-text)", border: "none", borderRadius: 10, padding: "12px 22px", fontFamily: "var(--sans, system-ui, sans-serif)", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
      ...
      <TweakColor label="Highlights" value={t.accent}
        options={[
          { value: "var(--accent)", label: "Blue-750" },
      ...
      <TweakColor label="CTA fill" value={t.action}
        options={[
          { value: "#000F47", label: "Blue-1000 · Midnight (default)" },
          { value: "#FFBF00", label: "Gold-750" },
          { value: "var(--accent)", label: "Blue-600 · Accent" },
          { value: "color-mix(in srgb, var(--primary) 9%, #ffffff)", label: "Blue-250 · Sky Blue" },
