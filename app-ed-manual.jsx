        ...
        {MN_STEPS.map((l, i) => {
          ...
          return (
            <span key={l} style={{ display: "inline-flex", alignItems: "center", gap: 7, borderRadius: 999, padding: "7px 14px",
              ...
              {done ? <I.check size={14} /> : <span style={{ fontSize: 11.5, opacity: on ? 1 : .7 }}>{i + 1}</span>}{l}
            </span>
          );
        })}
      </div>
...
function MnGettingStarted({ onNext }) {
  ...
  return (
    <div>
      <h1 className="serif" style={{ fontSize: 28, color: eMID, lineHeight: 1.1, margin: "0 0 8px" }}>Getting Started</h1>
...
function MnAddSkills({ sel, setSel, onBack, onNext }) {
  ...
  return (
    <div>
      <h1 style={{ fontFamily: "var(--sans)", fontSize: 21, fontWeight: 700, color: eMID, margin: "0 0 20px" }}>Add Skills</h1>
      ...
      {MN_CATS.map((c, ci) => (
        <div key={ci} style={{ background: "var(--card)", border: "1px solid " + eLINE, borderRadius: 8,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            <div style={{ width: 46, height: 46, borderRadius: "50%", background: "var(--status-neutral-bg)", color: eMID, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{React.createElement(I[c.icon], { size: 22 })}</div>
            <div>
          ...
          {c.suggest.filter((s) => !sel[ci].includes(s)).length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
              ...
              {c.suggest.filter((s) => !sel[ci].includes(s)).map((s) => (
                <button key={s} onClick={() => addSug(ci, s)} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "var(--card)", border: "1px solid " + eLINE, borderRadius: 8, padding: "8px 13px", fontFamily: "var(--sans)", fontSize: 15, color: eMID, cursor: "pointer" }}><I.plus size={14} /> {s}</button>
              ))}
            </div>
          )}
        </div>
        ...
        {rows.map((r, i) => (
          <div key={r.name} className="mn-rate-row" style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 20px", borderTop: i ? "1px solid " + eLINE : "none" }}>
            ...
            <div style={{ width: 170, flexShrink: 0, display: "flex", alignItems: "center", gap: 9 }}>
              <span style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--status-neutral-bg)", color: eMID, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        <button onClick={onFinish} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "var(--card)", border: "1px solid " + eLINE, borderRadius: 10, padding: "9px 16px", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 700, color: eMID, cursor: "pointer" }}>Answer Later</button>
      </div>
      ...
      <div key={i} className="mn-card" style={{ "--mn-from": dir > 0 ? "18px" : "-18px", position: "relative", zIndex: 10,
        ...
        <div style={{ display: "inline-flex", alignItems: "center", fontFamily: "var(--sans)", fontSize: 15, fontWeight: 400, color: eMID, background: "transparent", border: "1px solid #94918C", borderRadius: 8, padding: "6px 12px", marginBottom: 16 }}>
          Question {i + 1} of {QS.length}
        </div>
        <h3 style={{ fontFamily: "var(--sans)", fontSize: 21, fontWeight: 700, color: eMID, margin: "0 0 12px", lineHeight: 1.45 }}>
          {q.q}{q.req && <span style={{ color: "var(--danger)", marginLeft: 3 }}>*</span>}
          {!q.req && <span style={{ fontFamily: "var(--sans)", fontSize: 11.5, fontWeight: 400, color: "var(--ink)", background: "var(--status-neutral-bg)", borderRadius: 6, padding: "4px 10px", marginLeft: 8 }}>Optional</span>}
        </h3>
        ...
        <div style={{ border: "1.5px solid " + (tried && blocked ? "var(--danger)" : "var(--field-line)"), borderRadius: 2, overflow: "hidden", background: "var(--card)" }}>
          <textarea autoFocus value={text} onChange={(e) => { const v = e.target.value; setAnswers((a) => ({ ...a, [i]: v })); }}
            ...
            style={{ width: "100%", boxSizing: "border-box", padding: "14px 16px", border: "none", outline: "none", resize: "vertical", background: "transparent", fontFamily: "var(--sans)", fontSize: 15, color: eINK, lineHeight: 1.6, display: "block" }} />
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 18, padding: "8px 16px", borderTop: "1px solid " + eLINE, background: "rgba(0,15,71,.02)" }}>
            ...
            <span style={{ fontFamily: "var(--sans)", fontSize: 15, color: eMUT }}>Characters : {chars}</span>
          </div>
        </div>

        {(min || max) > 0 && (
          <div style={{ fontFamily: "var(--sans)", fontSize: 15, color: tried && blocked ? "var(--danger)" : eMUT, marginTop: 8 }}>
        ...
        <div style={{ display: "inline-flex", alignItems: "center", gap: 12 }}>
          ...
          <EdBtn primary small onClick={next}>
            {i === QS.length - 1
              ...
              : <React.Fragment>Next question <I.arrow size={15} /></React.Fragment>}
          </EdBtn>
        </div>
      </div>
    </div>
